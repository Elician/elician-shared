...
export class InputCode extends React.Component {
  ...
  render() {
    return (
      <div className={clsx(s.overlay, this.props.className, this.props.loading && s.disabled)}>
        <div className={s.wrapper} style={this.props.margin === 'disable' ? { margin: 0 } : null}>
          {[ ...Array(Number(this.props.length)) ].map((_, idx) => (
            <React.Fragment key={idx}>
              ...
              {idx === 2 ? <div className={clsx(s.separator, this.props.error && s.error)} /> : null}
            </React.Fragment>
          ))}
        </div>
        {this.props.error || this.props.tooltip ? (
          <div className={clsx(s.tooltip, this.props.error && s.error)}>
